...
//   lcd_init();
   ...
   if(b[i+1]=='\0')
   {
       data=I2C_master_read(1);// 0 ack
   }
   else
   {
       data=I2C_master_read(0);// 1 not ack
   }
   send_data(data);
   }
  ...
  // I2C_master_read(0);// 0 ack, 1 not ack
   ...
   send_cmd(0x01);
   I2C_master_start();
   I2C_master_write(0xa0);// 1010 1110
   I2C_master_write(0x00);
   I2C_master_write(0x00);
   for(i=0;c[i]!='\0';i++)
   {
       I2C_master_write(c[i]);
   }
   I2C_master_stop();
   
   __delay_ms(1000);
   I2C_master_start();
   show_data("Recieved data");
   I2C_master_write(0xa0);// 1010 1110
   I2C_master_write(0x00);
   I2C_master_write(0x00);
   I2C_master_repeated_start();
   I2C_master_write(0xa1); // 1010 1111
   send_cmd(0xc0);
   for(i=0;c[i]!=0;i++)
   {
   if(c[i+1]=='\0')
